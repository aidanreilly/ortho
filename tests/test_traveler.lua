...
PathGrid.add(g1, "p", 1, 1, 3, 1, "vertical_first") -- straight line, 3 cells
local tr1 = Traveler.new(1, 1)

local r1 = Traveler.step(tr1, g1, rng_first) -- resolves root, no movement yet
t.assert_eq(r1.note, false, "first step just resolves root, no note")
t.assert_eq(tr1.x, 1, "still sitting at root after resolve")

local r2 = Traveler.step(tr1, g1, rng_first) -- moves to cell 2
t.assert_eq(r2.note, false, "middle cell is not a dead end")
t.assert_eq(tr1.x, 2, "moved to 2nd cell")

local r3 = Traveler.step(tr1, g1, rng_first) -- moves to cell 3 (dead end)
...
local r4 = Traveler.step(tr1, g1, rng_first) -- re-resolves root
...
PathGrid.add(g2, "to-b", 1, 1, 3, 1, "vertical_first")
PathGrid.add(g2, "to-c", 1, 1, 1, 3, "vertical_first")

local tr_first = Traveler.new(1, 1)
Traveler.step(tr_first, g2, rng_first) -- occurrences_at root sorted by insertion: to-b added first
t.assert_eq(tr_first.path_id, "to-b", "rng_first picks the first-inserted occurrence")

local tr_last = Traveler.new(1, 1)
Traveler.step(tr_last, g2, rng_last)
...
PathGrid.add(g3, "h", 1, 2, 4, 2, "horizontal_first")
PathGrid.add(g3, "v", 2, 1, 2, 4, "vertical_first")
local tr3 = Traveler.new(1, 2) -- starts on path h
Traveler.step(tr3, g3, rng_first) -- resolve root (only occurrence is h, index1)
t.assert_eq(tr3.path_id, "h", "root of h has only one occurrence")
Traveler.step(tr3, g3, rng_last) -- step to (2,2), a decision cell; rng_last picks the later-inserted occurrence (v)
